...
import unittest
from array_challenge import my_map, my_filter, my_reduce, flatten_array, group_by, chain_operations

class TestArrayChallenge(unittest.TestCase):
    
    def test_my_map(self):
        self.assertEqual(my_map([1, 2, 3], lambda x: x * 2), [2, 4, 6])
        self.assertEqual(my_map(["hello", "world"], lambda x: x.upper()), ["HELLO", "WORLD"])
        self.assertEqual(my_map([], lambda x: x * 2), [])
    
    def test_my_filter(self):
        self.assertEqual(my_filter([1, 2, 3, 4], lambda x: x % 2 == 0), [2, 4])
        self.assertEqual(my_filter(["apple", "banana", "cherry"], lambda x: len(x) > 5), ["banana", "cherry"])
    ...
    def test_my_reduce(self):
        ...
        self.assertEqual(my_reduce(["a", "b", "c"], lambda acc, curr: acc + curr, ""), "abc")
    ...
    def test_chain_operations(self):
        ...
        self.assertEqual(chain_operations([2, 4, 6]), 56)     # 2² + 4² + 6² = 4 + 16 + 36 = 56
